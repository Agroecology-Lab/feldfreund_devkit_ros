...
class RelPosnedHeadingShim(Node):
    ...
    def _cb(self, msg: UBXNavRelPosNED) -> None:
        ...
        # Pack yaw into a quaternion (roll=0, pitch=0, yaw=yaw_enu)
        # q = [sin(yaw/2), 0, 0, cos(yaw/2)] in (x, y, z, w) convention
        half = yaw_enu * 0.5
        qz   = math.sin(half)
        qw   = math.cos(half)
        ...
        # Heading only — roll and pitch are zero
        out.orientation.x = 0.0
        out.orientation.y = 0.0
        out.orientation.z = qz
        out.orientation.w = qw

        # orientation_covariance is row-major 3×3 [roll, pitch, yaw]
        # Set yaw variance in [8]; mark roll/pitch unknown with -1 diagonal
        cov = [0.0] * 9
        cov[0] = -1.0   # roll unknown
        cov[4] = -1.0   # pitch unknown
        cov[8] = yaw_var
        out.orientation_covariance = cov

        # angular_velocity and linear_acceleration not provided
