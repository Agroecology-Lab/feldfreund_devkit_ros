...
| M3 | Open-field row-crop scenario | Live node-drop workflow implemented in UI: validates name + pose, writes updated YAML to `/workspace/maps/`, calls `switch_topological_map` with fallback to direct topic publish. Pending: tmap2 authoring from real field survey. | ~50% Done | 2026 |
