...
| M2a | fusioncore nav2 bridge | [fusioncore](https://github.com/manankharwar/fusioncore) provides the Nav2 topic remapping shim between LCAS `topological_navigation` and Nav2. Cloned and building in Dockerfile. Replaces the previously hand-rolled remapping approach and unblocks Issue #8. Pending: end-to-end test on live hardware. | In progress | 2026 |
| M3 | Open-field row-crop scenario | `strawberry_polytunnel.tmap2.yaml` (Riseholme) serves as format reference. Caatinga farm maps. Pending tmap2 authoring from real field survey. | ~40% Done | 2026 |
| M4 | RTK-GNSS localisation | Dual u-blox F9P moving-base configuration. `ublox_mb+r_rover` publishes `/rover/ublox_nav_sat_fix_hp` and `/rover/ubx_nav_rel_pos_ned` (NAV-RELPOSNED heading vector). `topic_tools relay` bridges fix to `/gnss/fix`; `relposned_heading_shim` converts NAV-RELPOSNED to `sensor_msgs/Imu` on `/gnss/heading` (yaw in orientation quaternion, `covariance[8]` = σ² from accuracy field). [fusioncore](https://github.com/manankharwar/fusioncore) UKF (`fusioncore_ros`) fuses `/gnss/fix` + `/gnss/heading` + `/odom/wheels` → `/fusion/odom` + `odom→base_link`  Heading to validate at antenna separation ≥ 0.5 m. Pending: antenna lever-arm offset measurement (`fusioncore.yaml`), test on live hardware. | ~60% Done | 2026 |
...
| M7 | Sentor safety monitoring | [LCAS Sentor](https://github.com/LCAS/sentor) topic and node health monitoring. Publishes `/safety/heartbeat` (false on estop/bumper trigger or driver loss) and `/warning/heartbeat` (false on camera, odometry, battery, or perception loss). Installed and building. `sowbot_monitor.yaml` authored and wired into `devkit.launch.py`. Pending: smoke-test against live hardware, BMS voltage cutoff confirmation. | ~40% Done | 2026 |
| M8 | Visual crop-row navigation | `sowbot_row_follow` package in [caatingarobotics](https://github.com/samuk/caatingarobotics). ExG vegetation index + Otsu threshold, scan-window line fitting, Cherubini & Chaumette visual servoing. Dervived from Visual servoing-based navigation for monitoring row-crop fields - Ahmadi, Alireza and Nardi, Lorenzo and Chebrolu. Validated on USB camera at 10 Hz: `/aoc/conditions/row_offset` and `/aoc/heartbeat/neo_vision` publishing live. Camera calibration (`camera_height_m`, `camera_tilt_deg`) required before field use; currently using BonnBot defaults. | ~60% Done | 2026 |
